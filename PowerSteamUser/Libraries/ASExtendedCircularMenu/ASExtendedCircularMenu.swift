...
//  https://github.com/anshul18895/ASExtendedCircularMenu
...
public class ASCircularMenuButton: UIButton {
    ...
    public var menuButtonSize: MenuButtonSize = .medium{
        didSet{
            ...
            case .small:
                ...
                break
            case .medium:
                self._menuButtonSize = CGSize.init(width: 45, height: 45)
                break
            case .large:
    ...
    public func setupCoverView() {
        // setup cover view
        coverView.frame = CGRect(origin: CGPoint(x: 0, y: 0), size: self.parentViewOfMenuButton.frame.size)
        coverView.backgroundColor = UIColor.clear
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(tapCover(sender:)))
        coverView.addGestureRecognizer(tapGesture)
        coverView.isUserInteractionEnabled = true
        coverView.isHidden = true
        self.parentViewOfMenuButton.insertSubview(coverView, belowSubview: self)
    }
    
    @objc func tapCover(sender: UITapGestureRecognizer) {
        if isSelected {
            onClickMenuButton()
        }
    }
    ...
    @objc public func onClickMenuButton(){
        ...
        if !isSelected {
            ...
            for i in 0...numberOfMenuItems - 1 {
                ...
                UIView.animate(withDuration: 0.15, delay: 0 * Double(i), options: .curveEaseIn, animations: {
    ...
    private func rotate360Degrees(duration: CFTimeInterval = 0.2, completionDelegate: AnyObject? = nil , isClockwise: Bool) {
        ...
        if isClockwise{
            rotateAnimation.toValue = CGFloat(1.25 * Double.pi)
        }else{
            rotateAnimation.toValue = CGFloat(-1.25 * Double.pi)
        }
    ...
    private func findAngles()->[CGFloat]{
        ...
        case .centerBottom:
            ...
            break
        default:
            let anglePartSize = (CGFloat(Double.pi) / CGFloat(1.2)) / CGFloat(numberOfMenuItems - 1)
            for i in 0...numberOfMenuItems-1{
                array.append( CGFloat(i) * anglePartSize)
            }
            break
